...
anes_oct1 <- read_excel("ANES_geoOct1.xlsx") #Google Trends
...
anes_j_oct1 <- left_join(anes1, anes_oct1, by = "statename")
```

```{r}
full_nov3 <- left_join(anes_j_nov3, kff, by = "statename")
full_oct1 <- left_join(anes_j_oct1, kff, by = "statename")
```

```{r}
#full_nov3$norm_bg[full_nov3$norm_bg == 0] = NA
#full_nov3$norm_5g[full_nov3$norm_5g == 0] = NA
#full_nov3$norm_ch[full_nov3$norm_ch == 0] = NA
```
...
```{r}
model1_bg = lm(`mean(V202331x)` ~ norm_bg + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model1_bg)
...
```{r}
######Present this first
model1_5g = lm(`mean(V202331x)` ~ norm_5g + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model1_5g)
...
```{r}
model1_ch= lm(`mean(V202331x)` ~ norm_ch + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model1_ch)
...
```{r}
full_nov3$norm_agg <- full_nov3$norm_bg + full_nov3$norm_5g + full_nov3$norm_ch
model1_agg= lm(`mean(V202331x)` ~ norm_agg + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model1_agg)
...
```{r}
full_nov3$int_bg <- full_nov3$mask_oct_19_2020*full_nov3$norm_bg
model2_bg = lm(`mean(V202331x)` ~ norm_bg + int_bg + mask_oct_19_2020 + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model2_bg)
...
```{r}
####This is the one too
full_nov3$int_5g <- full_nov3$mask_oct_19_2020*full_nov3$norm_5g
model2_5g = lm(`mean(V202331x)` ~ norm_5g + int_5g + mask_oct_19_2020 + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model2_5g)
...
```{r}
full_nov3$int_ch <- full_nov3$mask_oct_19_2020*full_nov3$norm_ch
model2_ch= lm(`mean(V202331x)` ~ norm_ch + int_ch + mask_oct_19_2020 + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model2_ch)
...
```{r}
full_nov3$int_agg <- full_nov3$mask_oct_19_2020*full_nov3$norm_agg
model2_agg= lm(`mean(V202331x)` ~ norm_agg + int_agg + mask_oct_19_2020 + casepermil + deathpermil + pop + govparty, data = full_nov3)
summary(model2_agg)
...
summary(model2_agg)
```
Moderation Plot with norm_5g:
```{r}
library(ggthemes)
ns5g_old <- ggplot(full_nov3, aes(x = norm_5g, y = `mean(V202331x)`)) + geom_point() + 
  geom_smooth(method='lm') + labs(x = "5G Conspiracy Google Trends") + labs(y = "Vaccine Hesitancy")
ns5g_old

s5g_old <- ggplot(full_nov3, aes(x = norm_5g, y = `mean(V202331x)`, colour = as.factor(mask_oct_19_2020))) + geom_point() + 
  geom_smooth(method='lm', aes(group = mask_oct_19_2020), se = FALSE)+ labs(x = "5G Conspiracy Google Trends") + labs(y = "Vaccine Hesitancy") + labs(colour = "Mask Mandate")
s5g_old
...
s5g <- ggplot(full_nov3_norm, aes(x = norm_5g, y = mean.V202331x., colour = as.factor(mask_oct_19_2020))) + geom_point() + 
  geom_smooth(method='lm', aes(group = mask_oct_19_2020), se = FALSE) + labs(x = "5G Conspiracy Google Trends") + labs(y = "Vaccine Hesitancy") + labs(colour = "Mask Mandate") + theme_economist(base_size = 13)
s5g

```

Moderation Plot with norm_agg:
```{r}
ggplot(full_nov3, aes(x = norm_agg, y = `mean(V202331x)`, colour = mask_oct_19_2020)) + geom_point() + 
  geom_smooth(method='lm', aes(group = mask_oct_19_2020), se = FALSE)


ggplot(full_nov3_norm, aes(x = norm_agg, y = mean.V202331x., colour = as.factor(mask_oct_19_2020))) + geom_point() + 
  geom_smooth(method='lm', aes(group = mask_oct_19_2020), se = FALSE)
```
Stargazer plot for presentation
```{r}
library(stargazer)
